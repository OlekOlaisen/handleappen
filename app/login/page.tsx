...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const redirectPath = sessionStorage.getItem("redirectAfterLogin") || "/week-planner"
      ...
      if (error.message === "Email not confirmed") {
        setError(
          "Du må bekrefte e-postadressen din før du kan logge inn. Sjekk innboksen din for en bekreftelseslenke.",
        )
      } else {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      setError("Det oppstod en feil ved innlogging med Google. Vennligst prøv igjen.")
    } finally {
      ...
      <div className="w-full max-w-md space-y-2">
        ...
        <div className="flex items-center justify-center gap-1 text-sm text-muted-foreground mb-4">
          ...
          <a
            ...
            className="inline-flex items-center justify-center bg-[#0A66C2] text-white w-5 h-5 rounded-sm hover:bg-[#004182] transition-colors"
          >
            <span className="font-semibold text-[11px]">in</span>
          </a>
        </div>
