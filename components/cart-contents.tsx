...
export default function CartContents() {
  const { items, orphanedItems, removeItem, updateQuantity, clearCart, isLoading, isSyncing } = useCart()
  const { user } = useAuth()
  ...
  Object.keys(storeData).forEach((storeCode) => {
    storeData[storeCode].hasAllProducts = storeData[storeCode].productCount === totalProductCount
  })
        ...
        <Alert className="mb-4">
          ...
          <AlertDescription className="mt-2 space-y-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-[#BE185D]">{bestStore.name}</span>
              <span>
        ...
        <div className="space-y-6">
          {items.length > 0 && (
            <div className="space-y-4">
              <h3 className="font-medium text-sm">Handlekurv - {bestStore?.name || ""}</h3>
              <div className="space-y-4">
                {items.map((item) => {
                  const bestStoreOption = item.storeOptions.find((opt) => opt.store.code === bestStore?.storeCode)
            ...
            <div className="mb-4">
              ...
              <div className="max-h-[250px] overflow-y-auto pr-1 space-y-2">
                {sortedStores.slice(0, 4).map((store) => (
                  <div key={store.storeCode} className="flex items-center justify-between text-sm mb-2">
                    <div className="flex items-center gap-2">
                      {store.logo && (
                        <div className="relative w-6 h-6 overflow-hidden rounded">
                          <Image
                      ...
                      <span className={store === bestStore ? "font-medium text-[#BE185D]" : ""}>
