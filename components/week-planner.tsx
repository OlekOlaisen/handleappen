...
import { format, getWeek, getYear, addDays, startOfWeek } from "date-fns"
...
import type { DayOfWeek } from "@/types/week-planner"
...
export default function WeekPlanner() {
  const { currentPlan, isLoading, addMealToDay, removeMealFromDay } = useWeekPlanner()
  const { meals } = useMeal()
  ...
  if (!currentPlan || !currentPlan.days) {
    return (
      <div className="flex items-center justify-center h-[50vh]">
        <p className="text-muted-foreground">No week plan available</p>
      </div>
    )
  }

  // Calculate all unique products and their total quantities across all meals in the week
  const getAllProducts = () => {
    const productMap = new Map<string, { product: any; quantity: number }>()

    currentPlan.days.forEach((day) => {
      day.meal_ids.forEach((mealId) => {
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Uke {currentWeek} · {format(weekStart, "MMMM yyyy", { locale: nb })}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <div className="text-sm text-muted-foreground">Total: {formatPrice(weekTotal)} kr</div>
          <Button onClick={handleAddAllToCart} disabled={isAddingToCart || weekTotal === 0}>
      ...
      <div className="grid gap-4 md:grid-cols-7">
        {DAYS_OF_WEEK.map((dayOfWeek, index) => {
          ...
          const day = currentPlan.days.find((d) => d.day_of_week === dayOfWeek)
          if (!day) return null
          ...
          return (
            <Card key={dayOfWeek} className="relative">
              <CardHeader className="pb-3">
                <CardTitle className="flex justify-between items-baseline text-base">
                  <span>{format(date, "EEEE d.", { locale: nb })}</span>
                    ...
                    {dayMeals.length > 0 ? (
                      <div className="space-y-2">
                        {dayMeals.map((meal) => (
                          <div
                            key={meal.id}
                            className="group relative rounded-md border bg-card/50 p-2 pr-8 text-sm hover:bg-muted/50 transition-colors"
                          >
                            ...
                            <Button
                              ...
                              onClick={() => removeMealFromDay(dayOfWeek, meal.id)}
                            >
                        ...
                        <Dialog>
                          ...
                          <DialogContent>
                            <DialogHeader>
                              ...
                              <DialogDescription>
                                Velg et måltid å legge til {format(date, "EEEE", { locale: nb })}
                              </DialogDescription>
                            </DialogHeader>
                              ...
                              <div className="grid gap-2 py-4">
                                {meals.map((meal) => (
                                  <Button
                                    ...
                                    onClick={() => {
                                      addMealToDay(dayOfWeek, meal.id)
